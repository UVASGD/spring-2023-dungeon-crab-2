...
{
    public List<float> waterLevelHeights;
    ...
    void Start()
    {
        ...
        if(waterLevelHeights.Count == 0)
        {
            waterLevelHeights.Add(transform.position.y);
        }
        transform.position = new Vector3(transform.position.x, waterLevelHeights[gm.waterLevel], transform.position.z);
    }

    // Update is called once per frame
    void FixedUpdate()
    {
        transform.position = Vector3.MoveTowards(transform.position, new Vector3(transform.position.x, waterLevelHeights[gm.waterLevel], transform.position.z), speedOfChange);
    }
}
